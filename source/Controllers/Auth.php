...
class Auth extends Controller
{
    ...
    {
        $userLogin = filter_var($data["user"], FILTER_DEFAULT);
        $passwd = filter_var($data["passwd"], FILTER_DEFAULT);

        if (!$userLogin || !$passwd) {
            echo $this->ajaxResponse("message", [
                "type" => "alert",
                "message" => "Informe seu usuario e senha para logar!"
            ]);
            return;
        }

        $user = (new User())->find("user = :e", "e={$userLogin}")->fetch();
        if (!$user || !password_verify($passwd, $user->passwd)) {
            echo $this->ajaxResponse("message", [
                "type" => "error",
                "message" => "Usuario ou senha informados não conferem"
            ]);
            return;
        }
